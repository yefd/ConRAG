...
def get_instruction_dataset(dataset, max_prompt_length, tokenizer, retrieval_aware, use_cot, RETRIEVAL_TOKEN, add_noise, noise_type, gt_position, noise_num, standard_prompt, retrieval_aware_type, num_k, sample_answer=True):
    instruction_dataset = []
...
def load_bioasq_data(input_path):
    # download from https://huggingface.co/datasets/enelpol/rag-mini-bioasq
...
def load_bioasq_dataset(input_path, max_prompt_length, tokenizer, retrieval_aware=True, use_cot=False, RETRIEVAL_TOKEN='<R>', add_noise=False, noise_type='', gt_position='', noise_num=100, use_emb=True, standard_prompt=False, retrieval_aware_type='', num_k=10, ignore_train=False):
    train_data, test_data = load_bioasq_data(input_path)
    instruction_dataset_train = get_instruction_dataset(train_data, max_prompt_length, tokenizer, retrieval_aware, use_cot, RETRIEVAL_TOKEN, add_noise, noise_type, gt_position, noise_num, standard_prompt, retrieval_aware_type, num_k) if not ignore_train else None
    instruction_dataset_test = get_instruction_dataset(test_data, max_prompt_length, tokenizer, retrieval_aware, use_cot, RETRIEVAL_TOKEN, add_noise, noise_type, gt_position, noise_num, standard_prompt, retrieval_aware_type, num_k)
