    ...
    if not add_noise:
        for document_index, document in enumerate(documents[:num_k]):
            if retrieval_aware and not standard_prompt:
                ...
                document_prompt = f"{RETRIEVAL_TOKEN}{RETRIEVAL_TOKEN}{document['text']}"
...
def get_instruction_dataset(dataset, idx, max_prompt_length, tokenizer, retrieval_aware, use_cot, prompt_type, RETRIEVAL_TOKEN, add_noise, noise_type, gt_position, noise_num, standard_prompt, retrieval_aware_type, num_k, sample_answer=True):
    ...
    for i in tqdm(idx):
        ...
        question = input_example["query"]
        input_example['question'] = question
        documents = []
...
def load_mrag_data(input_path, dataset_seed=42):
    ...
    if input_path.endswith('.json'):
        ...
        with open(input_path, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                data['instruction'] = prompt_template.format(question=data['query'], search_results=data['compressed_prompt'])
                dataset.append(data)
            f.close()
...
def load_mrag_dataset(input_path, max_prompt_length, tokenizer, retrieval_aware=True, use_cot=False, prompt_type='', RETRIEVAL_TOKEN='<R>', add_noise=False, noise_type='', gt_position='', noise_num=100, use_emb=True, standard_prompt=False, retrieval_aware_type='', num_k=10, dataset_seed=42, ignore_train=False):
    if 'compress' in input_path:
        return None, load_compress_data(input_path)
    examples, train_index, test_index = load_mrag_data(input_path, dataset_seed)
    instruction_dataset_train = get_instruction_dataset(examples, train_index, max_prompt_length, tokenizer, retrieval_aware, use_cot, prompt_type, RETRIEVAL_TOKEN, add_noise, noise_type, gt_position, noise_num, standard_prompt, retrieval_aware_type, num_k) if not ignore_train else None
