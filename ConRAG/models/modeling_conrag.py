...
from transformers import  LlamaForCausalLM, LlamaModel, LlamaPreTrainedModel, PreTrainedModel, LlamaConfig, AutoModelForCausalLM, PretrainedConfig
from typing import Any, Dict, List, Optional, Tuple

class ConRAGLlamaConfig(PretrainedConfig):
    model_type = "wragllama"
...
class RFormer(nn.Module):
    def __init__(self, input_dim, num_k, d_model=256, n_head=4, num_layers=1, dropout=0.1, margin=1.0) -> None:
        super().__init__()
        self.input_dim = input_dim
        self.num_k = num_k
        self.d_model = d_model
        ...
        self.projector_layer = nn.Linear(in_features=d_model, out_features=d_model)
        self.triplet_loss = nn.TripletMarginLoss(margin=self.margin, p=2)  # `p=2` means EUCLIDEAN distance
    ...
    def forward(self, x, label=None):
        ...
        x = x + pe
        att_outputs = self.attention_layer(x.permute(1, 0, 2)).permute(1, 0, 2)

        if self.training:
            y = self.classifier_layer(att_outputs)
            if label is not None:
                label = label.to(y.dtype)
            loss = self.classification_loss(y, label) if self.training else None

            pos_mask = (1 - label).reshape(x.shape[0], x.shape[1]).bool() # Pos marked in False (or 0) is a valid token and should be attended
            neg_mask = label.reshape(x.shape[0], x.shape[1]).bool() # Pos marked in True (or 1) represents a padding token and should be ignored
            pos_outputs = self.attention_layer(x.permute(1, 0, 2), src_key_padding_mask=pos_mask).permute(1, 0, 2)
            neg_outputs = self.attention_layer(x.permute(1, 0, 2), src_key_padding_mask=neg_mask).permute(1, 0, 2)
            loss_triple = self.triplet_loss(att_outputs.mean(dim=1), pos_outputs.mean(dim=1), neg_outputs.mean(dim=1))

            loss += loss_triple
        else:
            loss = None
        return att_outputs, loss

class ConRAGLlamaForCausalLM(PreTrainedModel):
    config_class = ConRAGLlamaConfig
    ...
    def encode_inputs(self, 
        ...
        if embeds is not None:
            ...
            if input_embeds.dim() == 2:
                ...
                if replace_idx.dim() == 1:
                    replace_idx = replace_idx.unsqueeze(0)
    ...
    def save_model(self, save_directory):
        ...
        torch.save(model_dict, os.path.join(save_directory, 'ConRAGLlama_pytorch_model.bin'))
    ...
    def from_pretrained(cls, pretrained_model_path, *model_args, **kwargs):
        ...
        model_path = os.path.join(pretrained_model_path, 'ConRAGLlama_pytorch_model.bin')
